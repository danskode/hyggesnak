...
const providers = {
    giphy: {
        ...
        async trending() {
            const url = `https://api.giphy.com/v1/gifs/trending?api_key=${config.giphyApiKey}&limit=24&rating=g`;
            const data = await fetchJson(url, 'Giphy');
            return (data.data || []).map(mapGiphyGif);
        },
        async search(q) {
            const url = `https://api.giphy.com/v1/gifs/search?api_key=${config.giphyApiKey}&q=${encodeURIComponent(q)}&limit=24&rating=g`;
            const data = await fetchJson(url, 'Giphy');
            return (data.data || []).map(mapGiphyGif);
        }
    },
    heypster: {
        ...
        async trending() {
            const url = `https://api.heypster.com/v1/featured?key=${config.heypsterApiKey}&limit=24&media_filter=gif`;
            const data = await fetchJson(url, 'Heypster');
            return (data.results || []).map(mapHeypsterGif);
        },
        async search(q) {
            const url = `https://api.heypster.com/v1/search?key=${config.heypsterApiKey}&q=${encodeURIComponent(q)}&limit=24&media_filter=gif`;
            const data = await fetchJson(url, 'Heypster');
            return (data.results || []).map(mapHeypsterGif);
        }
    }
